...
public class Shoallet_Old : Enemy
{
    [SerializeField]
    ...
    void RichochetAttackUpdate()
    {
        ...
        //if ((state == 2) | (state == 3))
        //{
        //    RaycastHit2D hit = Physics2D.Raycast(transform.position, transform.up, .6f, 1 << LayerMask.NameToLayer("Wall"));
    ...
    // args[0] is a string with the contactEdge, args[1] is a bool on whether it hit a shield or not
    //void Contact(object[] args)
    //{
    //    string contactEdge = (string)args[0];
    //    bool hitShield = (bool)args[1];
